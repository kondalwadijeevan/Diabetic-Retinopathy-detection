...
main = tkinter.Tk()
main.title("Diabetic Retinopathy")
...
def getLabel(name):
    ...
    for i in range(len(labels)):
        if labels[i] == name:
            ...
            break
    return index
print("Diabetic Retinopathy Class Labels found in dataset : "+str(labels))
...
def trainCNN():
    ...
    #dataset preprocessing such as shuffling and normalization
    text.delete('1.0', END)
    #train eyenet model using CNN optimizer fixed learning rate
    ...
    print("Existing CNN Validation Accuracy  :  "+str(acc))
...
def trainResnet50():
    ...
    #dataset preprocessing such as shuffling and normalization
    text.delete('1.0', END)
    #training EyeNet with Resnet50 Optimizer
    ...
    y_val1 = np.argmax(y_val, axis=1)
    acc = accuracy_score(y_val1, predict) * 100
    print("Propose Resnet50 Validation Accuracy  :  "+str(acc))
    predict = eyenet_model.predict(X_test)
    predict = np.argmax(predict, axis=1)
    y_test1 = np.argmax(y_test, axis=1)
...
def trainHybrid():
    ...
    y_val1 = np.argmax(y_val, axis=1)
    acc = accuracy_score(y_val1, predict) * 100
    print("Extension Adam & Resnet50 Validation Accuracy  :  "+str(acc))
...
def graph():
    ...
    df = pd.DataFrame([['Existing CNN','Accuracy',accuracy[0]],['Existing CNN','Precision',precision[0]],['Existing CNN','Recall',recall[0]],['Existing CNN','FSCORE',fscore[0]],
                       ['Propose Resnet50','Accuracy',accuracy[1]],['Propose Resnet50','Precision',precision[1]],['Propose Resnet50','Recall',recall[1]],['Propose Resnet50','FSCORE',fscore[1]],
                       ['Extension Adam & Resnet50','Accuracy',accuracy[2]],['Extension Adam & Resnet50','Precision',precision[2]],['Extension Adam & Resnet50','Recall',recall[2]],['Extension Adam & Resnet50','FSCORE',fscore[2]],
...
font = ('times', 14, 'bold')
title = Label(main, text='A Lesion-Based Diabetic Retinopathy Detection Through Hybrid Deep Learning Model')
...
font1 = ('times', 13, 'bold')
RunCNNButton = Button(main, text="Train CNN", bg='dim gray', fg='white',command=trainCNN)
RunCNNButton.place(x=50,y=200)
RunCNNButton.config(font=font1)

font1 = ('times', 13, 'bold')
RunCNNButton = Button(main, text="Train Resnet50", bg='dim gray', fg='white',command=trainResnet50)
RunCNNButton.place(x=50,y=250)
RunCNNButton.config(font=font1)

font1 = ('times', 13, 'bold')
RunExtensionButton = Button(main, text="Train with Adam & Resnet50", bg='dim gray', fg='white',command=trainHybrid)
